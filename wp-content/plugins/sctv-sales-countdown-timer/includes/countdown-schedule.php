	...
	private function sctv_schedule_time_shortcode( $id ) {
		...
		if ( $index >= 0 ) {
			...
			if ( $countdown_time_end < $now ) {
				$time_div = $countdown_time + $loop_time_val;
				$time_div = $time_div == 0 ? 1 : $time_div;
				$missed                 = floor( ( $now - $countdown_time_end ) / $time_div );
				$countdown_time_start_t = $countdown_time_end + $missed * $time_div + $loop_time_val;
			} else {
...
//			$date_on_sale_to   = $product->get_meta( '_sale_price_dates_to', true );
			$date_on_sale_from = (int) get_post_meta( $id, '_sale_price_dates_from', true );
			$date_on_sale_to   = (int) get_post_meta( $id, '_sale_price_dates_to', true );
			$countdown_time    = (int) $date_on_sale_to - (int) $date_on_sale_from;
			if ( $loop_time_val != '' ) {
			...
			if ( $date_on_sale_to < $time_offset ) {
				$time_div = $countdown_time + $loop_time_val;
				$time_div = $time_div == 0 ? 1 : $time_div;
				$missed              = floor( ( $time_offset - $date_on_sale_to ) / $time_div );
				$date_on_sale_from_t = $date_on_sale_to + $missed * $time_div + $loop_time_val;
			} else {
