...
class VI_SCT_SALES_COUNTDOWN_TIMER_Admin_Checkout_Report {
	...
	public function admin_enqueue_scripts() {
		...
		if ( $page === 'sales-countdown-timer-checkout-report' ) {
			...
			$args = array(
				...
				'chart_labels'         => array(),
				'click_label'          => esc_html__( 'Click checkout button', 'sales-countdown-timer' ),
				'click_data'           => array(),
				'sct_order_label'      => esc_html__( 'Orders on sale countdown', 'sales-countdown-timer' ),
				'sct_order_data'       => array(),
				'order_label'          => esc_html__( 'Orders', 'sales-countdown-timer' ),
				'order_data'           => array(),
				'sct_time_order_label' => esc_html__( 'Orders', 'sales-countdown-timer' ),
				...
				if ( $data ) {
					...
					if ( count( $data->label ) ) {
						...
						foreach ( $data->label as $label ) {
							...
							$count_sct_order[] = $wpdb->get_var( $query_sct_orders );// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
							...
							$count_order[]     = $wpdb->get_var( $query_orders );// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
						}
					}
	...
	private function get_data_by_countdown_time1( $choose_time ) {
		...
		$args               = array(
			...
			'posts_per_page' => - 1,
			'meta_key'       => 'sctv_checkout_countdown_info',// phpcs:ignore WordPress.DB.SlowDBQuery.slow_db_query_meta_key
		);
	...
	private function get_data_by_date() {
		...
		if ( count( $files ) ) {
			foreach ( $files as $file ) {
				@$content = file_get_contents( VI_SCT_SALES_COUNTDOWN_TIMER_CACHE . $file . '.txt' );// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
				if ( $content ) {
