...
class VI_SCT_SALES_COUNTDOWN_TIMER_Frontend_Shortcode {
	...
	public function shortcode_init() {
		if ( isset( $_REQUEST['_woo_ctr_frontend_nonce'] ) && ! wp_verify_nonce( wc_clean( wp_unslash( $_REQUEST['_woo_ctr_frontend_nonce'] ) ), 'woo_ctr_frontend_nonce' ) ) {
			return;
		}
	...
	public function register_shortcode( $atts ) {
		extract( shortcode_atts( array(
			...
			'checkout_to_time'              => 0,
			'countdown_upcoming_enable'     => 0,
			'countdown_time_elapsed'        => 0,
			'elapsed_datetime_format'       => get_option('date_format') . ' - ' . get_option( 'time_format' ),
			'time_units'                    => '',
			'time_separator'                => '',
			'datetime_format'               => '',
		...
		switch ( $type ) {
			...
			default:
				...
				if ( ( $sale_from > $now || $sale_to < $now ) && ( ! isset( $countdown_upcoming_enable ) || ! $countdown_upcoming_enable ) &&
				     ( ! isset( $countdown_time_elapsed ) || ! $countdown_time_elapsed ) ) {
					$continue = false;
				}
				$end_time = $sale_to - $now;
				if ( $message ) {
					$text = $message;
                } elseif ( $countdown_upcoming_enable && $sale_from > $now ) {
					$end_time = $sale_from - $now;
					$text = $this->settings->get_current_countdown_language( 'sale_countdown_sc_upcoming_message', '_' . $language, $index );
                } elseif ( intval( $end_time ) <= 0 && $countdown_time_elapsed ) {
					$is_time_elapsed = true;
					$text = $this->settings->get_current_countdown_language( 'sale_countdown_sc_elapsed_message', '_' . $language, $index );
                } else {
					$text = $this->settings->get_current_countdown_language( 'sale_countdown_message', '_' . $language, $index );
                }
//				$text     = $message ? $message : $this->settings->get_current_countdown_language( 'sale_countdown_message', '_' . $language, $index );
		...
		/*message*/
        if ( $countdown_time_elapsed && $is_time_elapsed ) {
	        $text = explode( '{schedule_time}', $text );
        } else {
...
//		}
		...
		$div_container_class[]    = $countdown_time_elapsed && $is_time_elapsed ? 'woo-sctr-countdown-elapsed' : '';
        ...
        <div class="<?php echo esc_attr( $div_container_class ); ?>">
            <div class="<?php echo esc_attr( $countdown_layout_class ); ?>">
				...
				if ( $countdown_time_elapsed && $is_time_elapsed ) {
                    echo wp_kses_post( wp_date( $elapsed_datetime_format, $sale_to ) );
                } else {
							...
							if ( $post_product->has_child() ) {
								...
								if ( count( $product_children ) ) {
									foreach ( $product_children as $product_child ) {
										...
										$_id = $child_wc->get_meta( '_woo_ctr_select_countdown_timer', true );
									}
								}
							}
						}
...
//							}
//							try {
//								if ( eval( $assign_page ) ) {// phpcs:ignore Generic.PHP.ForbiddenFunctions.Found
//									$conditional_tags = true;
//								}
//							} catch ( Error $e ) {
...
//			}
		...
		$assign_request_url_data   = $this->settings->get_params( 'checkout_countdown_display_on_assign_request_url' );
		$assign_request_url        = false;
		...
		if ( is_plugin_active( 'woocommerce/woocommerce.php' ) && $this->settings->get_params( 'checkout_countdown_enable' ) ) {
			...
			if ( count( $checkout_display_on ) ) {
				...
				foreach ( $checkout_display_on as $item ) {
					switch ( $item ) {
						...
						case 'assign':
							...
							if ( self::check_assign_request_url( $assign_request_url_data ) ) {
								$assign_request_url = true;
								$page_show_t .= '||1';
							}
				...
				if ( $page_show_t ) {
					...
					try {
						if ( eval( $page_show_t ) ) {// phpcs:ignore Generic.PHP.ForbiddenFunctions.Found
		...
		if ( ! wp_script_is( 'woo-sctr-shortcode-countdown-script', 'registered' ) ) {
			wp_register_script( 'woo-sctr-shortcode-countdown-script', VI_SCT_SALES_COUNTDOWN_TIMER_JS . 'shortcode-countdown.' . $suffix . 'js', array( 'jquery' ), VI_SCT_SALES_COUNTDOWN_TIMER_VERSION, false );
			wp_localize_script(
...
//		}
	...
	public static function check_assign_request_url( $setting_assign ) {
		$setting_assign = explode( '||', $setting_assign );
		$site_url       = get_option( 'siteurl' );
		$current_url    = $_SERVER['REQUEST_URI'];
		$request_url    = wp_parse_url( $current_url, PHP_URL_PATH );
		$request_url    = urldecode( trim( $request_url, "/" ) );

		foreach ( $setting_assign as $s_key => $s_val ) {
			if ( $s_val == $request_url ) {
				return true;
			}
		}
