...
class VI_SCT_SALES_COUNTDOWN_TIMER_Frontend_Single_Product_Countdown {
		...
		if ( is_product() && is_single() ) {
			...
			if ( $product ) {
				if ( ! $product->is_type( 'variable' ) ) {
					...
					$variation_id      = $product->get_meta( '_woo_ctr_display_enable', true );
	...
	public function init_countdown( $product_id ) {
		...
		$id         = $wc_product->get_meta( '_woo_ctr_select_countdown_timer', true );
			...
			case 'single-product/sale-flash.php':
				if ( $this->position === 'before_saleflash' ) {
					...
					echo '<div class="woo-sctr-single-product-container woo-sctr-single-product-shortcode-' . esc_attr( $this->id ) . ( in_array( $this->pg_position, array(
							'left_countdown',
							'right_countdown'
						) ) ? ' woo-sctr-single-product-inline-container ' : '' ) . '">' . $shortcode . '</div>';// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				}
				break;
