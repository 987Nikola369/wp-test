...
if ( ! function_exists( 'woosc_update_checker' ) ) {
	...
	add_action( 'in_plugin_update_message-' . plugin_basename( WOOSC_FILE ), 'woosc_update_license', 99 );
	...
	function woosc_update_checker() {
		if ( $key = wpc_get_update_key( 5570 ) ) {
			...
			PucFactory::buildUpdateChecker( 'https://api.wpclever.net/update/5570.json', WOOSC_FILE, plugin_basename( WOOSC_DIR ) );
		}
	}

	function woosc_update_license( $plugin_data ) {
